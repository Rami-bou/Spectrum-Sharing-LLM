...
def secondary(state: GraphState) -> GraphState:
    ...
    if not state.get('P2') or sum(state['P2']) == 0:
        ...
        interference_on_secondary = [total_p1 * h for h in state['cross_secondary_channels']]
        gap = [inter - secondary_I_max for inter in interference_on_secondary]
        state['Secondary_gap'] = gap
        max_gap = max([inter - primary_I_max for inter in interference_on_secondary])
