...
def gen_channels(length):
    ...
    while len(data) < length:
        ...
        data.append([direct_h_primary, direct_h_secondary, cross_h_primary, cross_h_secondary, P1_dist, P2_dist, allowed_p2])
...
def primary(state: GraphState) -> GraphState:
    ...
    5. 2.0 dB < Margin <= 5.0 dB: Below capacity, wasting secondary power budget. decision=REJECT, action=INCREASE, severity=LOW.
    6. Margin > 5.0 dB: The primary is safe, and you have a small amount of excess room. decision=REJECT, action=INCREASE, severity=MEDIUM.
...
def calculate_secondary_se(P1_vector, P2_vector, direct_h_secondary, cross_h_secondary):
    """Continuous Shannon-proxy throughput for secondary receivers.
    Kept continuous (not discrete MCS) because the MCS table above is calibrated to the
    primary's baseline SINR range (~30 dB) and isn't reachable by secondary receivers in
    this topology — secondary SINR sits far below MCS tier 0 even with the full budget
    optimally allocated. A discrete tier-match metric here would be misleadingly perfect
    (everyone always "tier 0") rather than informative.
    """
    se = 0.0
    total_p1_interf_base = sum(P1_vector)
    for i in range(len(P2_vector)):
        signal = P2_vector[i] * direct_h_secondary[i]
        interference = total_p1_interf_base * cross_h_secondary[i]
...
def compute_worst_margin(P1_vector, P2_vector, direct_h_primary, cross_h_primary):
    """Deterministic recomputation of the primary's worst MCS margin (dB), independent
    of what the LLM self-reported, so protection can be checked objectively."""
    total_p2 = sum(P2_vector)
    margins = []
    for j in range(len(P1_vector)):
        signal = P1_vector[j] * direct_h_primary[j]
...
primary_protected_count = 0      # NEW
converged_count = 0              # NEW
all_three_served_count = 0       # NEW
one_receiver_only_count = 0      # NEW
se_standard_list = []            # NEW
se_optimal_list = []             # NEW
se_pred_secondary_list = []      # NEW

for i in range(len(test)):
    direct_h_pri = test[i][0]
    direct_h_sec = test[i][1]    # NEW (was missing before, needed for secondary SE)
    cross_h_pri = test[i][2]
    cross_h_sec = test[i][3]     # NEW
    true_p1 = test[i][4]
    true_p2 = test[i][5]
    allowed_p2 = test[i][6]      # NEW
    ...
    # --- NEW: primary protection, checked independently of the LLM's self-reported decision ---
    worst_margin = compute_worst_margin(true_p1, pred_p2, direct_h_pri, cross_h_pri)
    if worst_margin >= 0.0:
        primary_protected_count += 1

    # --- NEW: convergence vs forced stop ---
    if result['primary_decision'] == "ACCEPT":
        converged_count += 1

    # --- NEW: winner-take-all check ---
    served = sum(1 for p in pred_p2 if p > 0)
    if served == M:
        all_three_served_count += 1
    elif served == 1:
        one_receiver_only_count += 1

    # --- NEW: three-way secondary throughput comparison (Standard / Optimal / LLM) ---
    optimal_p2 = allocate_p2_knapsack_optimal(allowed_p2, direct_h_sec, cross_h_sec, true_p1)
    se_standard_list.append(calculate_secondary_se(true_p1, true_p2, direct_h_sec, cross_h_sec))
    se_optimal_list.append(calculate_secondary_se(true_p1, optimal_p2, direct_h_sec, cross_h_sec))
    se_pred_secondary_list.append(calculate_secondary_se(true_p1, pred_p2, direct_h_sec, cross_h_sec))

    print(f"Allocation P2 pred: {result['P2']}  (worst primary margin: {worst_margin:.2f} dB, decision: {result['primary_decision']})")
    print(f"Allocation P2 true: {test[i][5]}")


n_test = len(test)
print("\n" + "="*40)
print(" AGGREGATE NEGOTIATION BENCHMARK ")
print("="*40)
print(f"Primary protection rate (worst margin >= 0 dB): {primary_protected_count}/{n_test} ({100*primary_protected_count/n_test:.1f}%)")
print(f"Convergence rate (reached ACCEPT, not forced stop): {converged_count}/{n_test} ({100*converged_count/n_test:.1f}%)")
print(f"All {M} secondary receivers served (P2>0): {all_three_served_count}/{n_test} ({100*all_three_served_count/n_test:.1f}%)")
print(f"Only 1 secondary receiver served: {one_receiver_only_count}/{n_test} ({100*one_receiver_only_count/n_test:.1f}%)")

mean_se_standard = np.mean(se_standard_list)
mean_se_optimal = np.mean(se_optimal_list)
mean_se_pred = np.mean(se_pred_secondary_list)

print(f"\nSecondary throughput (Shannon-proxy SE, higher is better):")
print(f"  Standard (flat split):           {mean_se_standard:.3f}")
print(f"  Optimal (knapsack, same budget): {mean_se_optimal:.3f}")
print(f"  LLM-negotiated:                  {mean_se_pred:.3f}")
print(f"  Gain of LLM over Standard: {100*(mean_se_pred/mean_se_standard - 1):.1f}%")
print(f"  Gap of LLM vs Optimal:     {100*(1 - mean_se_pred/mean_se_optimal):.1f}%")
print("="*40 + "\n")
