...
def secondary(state:GraphState) -> GraphState:
    ...
    if not state['primary_critique']:
        ...
        resp = structured_critic.invoke([
            ...
            HumanMessage(content=f"""Output the maximum value 1000 for each receiver in the list. """
            
            # Complete thr following allocations depends on the channels:
...
if len(test) > 0:
    ...
    plt.savefig(
        os.path.join(
            RESULT_DIR,
            "secondary_rate_combined_attack.png"
        ),
        dpi=300,
        bbox_inches="tight"
    )
    ...
    plt.savefig(
        os.path.join(
            RESULT_DIR,
            "primary_interference_combined_attack.png"
        ),
        dpi=300,
        bbox_inches="tight"
    )
