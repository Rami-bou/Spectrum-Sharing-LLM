...
def primary(state: GraphState) -> GraphState:
    ...
    2. -3.0 dB <= Margin < -1.0 dB: action=DECREASE, severity=MEDIUM.
    3. -1.0 dB <= Margin < 0.0 dB: decision=REJECT, action=DECREASE, severity=LOW.
    4. 0.0 dB <= Margin <= 4.0 dB: decision=ACCEPT.
    5. 4.0 dB < Margin <= 7.0 dB: decision=REJECT, action=INCREASE, severity=LOW.
    6. Margin > 7.0 dB: Far below capacity, secondary is being overly conservative. decision=REJECT, action=INCREASE, severity=HIGH.
...
class SecondaryOutput(BaseModel):
    ...
    allocation_secondary: int = Field(description="Your allocation for your secondary receivers.")
...
def secondary(state:GraphState) -> GraphState:
    ...
    if not state['primary_critique']:
        ...
        The Primary Evaluator monitors the 'Worst MCS Margin' (in dB). The sweet spot is a margin exactly between 0.0 dB and 4.0 dB.
...
def build_prompt(train):
    ...
    for i in range(len(train)):
        ...
        Then the Power (P2) allocation are: {sum(train[i][5])}
        """
...
for i in range(len(test)):
    ...
    pred_p2 = allocate_p2_knapsack_optimal(result['P2'], direct_h_sec, cross_h_sec, true_p1)
    ...
    for j in range(len(test[i][0])):
        ...
        interference = result['P2'] * cross_h_prim[j]
        actual_sinr_linear = signal / (1.0 + interference)
        actual_sinr_db = 10 * math.log10(actual_sinr_linear) if actual_sinr_linear > 0 else -999
